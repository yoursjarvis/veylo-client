...
export const rbacKeys = {
  ...
  roles: (orgId: string, search?: string) =>
    [...rbacKeys.all, "roles", orgId, { search }] as const,
  assignments: (orgId: string) =>
    [...rbacKeys.all, "assignments", orgId] as const,
}
...
export const useOrganizationRoles = (orgId: string, search?: string) => {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) =>
      rbacService.getOrganizationRoles(orgId, {
        search,
        limit: 10,
        cursor: pageParam,
      }),
...
export const useUpdateRoleHierarchy = (orgId: string) => {
  ...
  return useMutation({
    mutationFn: (roleIds: string[]) =>
      rbacService.updateRoleHierarchy({ roleIds, organizationId: orgId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["rbac", "roles", orgId] })
    },
  })
