...
export const rbacService = {
  ...
  getOrganizationRoles: async (
    ...
    const { data: axiosResponse } = await axiosInstance.get(`/rbac/organizations/${orgId}/roles`, {
    ...
    let roles = rawRoles.map((r: any) => {
      ...
      if (r.assignments) {
        ...
        users = r.users ?? Array.from({ length: userCount }).map((_, j) => ({
          id: `user-${r.id}-${j}`,
          name: `User ${r.id}-${j}`,
    ...
    const startIndex = params?.cursor ? roles.findIndex(r => r.id === params.cursor) + 1 : 0;
    const endIndex = startIndex + limit;

    const data = roles.slice(startIndex, endIndex);
    const nextCursor = endIndex < roles.length ? data[data.length - 1]?.id : undefined;
