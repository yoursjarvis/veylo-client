...
export function useWorkspaceObjectives(workspaceId: string, withTrashed = false) {
...
export function useForceDeleteObjective(workspaceId: string) {
  ...
  return useMutation({
    ...
    onError: (err: { response?: { data?: { message?: string } } }) => {
      toast.error(err.response?.data?.message || "Failed to permanently delete objective");
    },
  });
}
