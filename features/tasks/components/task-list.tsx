    ...
    >
      <div className="px-1 py-2 flex items-center justify-center">
        <div
          {...(dragHandleProps || {})}
          {...(dragHandleListeners || {})}
          className="opacity-0 group-hover:opacity-100 cursor-grab active:cursor-grabbing hover:bg-muted rounded p-0.5 transition-opacity text-muted-foreground"
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isDragging && "z-50 bg-muted/40 border-y border-dashed border-primary/40 shadow-inner"
      )}
    >
...
export function StatusSection({
  ...
  if (tasks.length === 0) {
    return (
      <div className={cn("flex flex-col mb-4", isOver && "bg-primary/5 rounded-md outline outline-1 outline-primary/30")} ref={setNodeRef}>
        <div 
        ...
        {!isCollapsed && (
          <div className={cn(
            "h-12 border-2 border-dashed border-border/40 mx-2 my-2 rounded-md flex items-center justify-center text-[11px] font-medium text-muted-foreground/50 transition-colors",
            isOver && "border-primary/40 text-primary/70 bg-primary/5"
          )}>
            Drop tasks here
          </div>
        )}
  ...
  return (
    <div className={cn("flex flex-col pb-4", isOver && "bg-primary/5 outline outline-1 outline-primary/20")} ref={setNodeRef}>
      <div 
...
export function TaskList({
  ...
  const [listTasks, setListTasks] = useState<Task[]>(() => [...initialTasks].sort((a,b) => (a.position||0) - (b.position||0)));
  const [activeTask, setActiveTask] = useState<Task | null>(null);
  const updateTaskOrderMutation = useUpdateTaskOrder(projectId);

  if (initialTasks !== prevTasks) {
    setPrevTasks(initialTasks);
    setListTasks([...initialTasks].sort((a,b) => (a.position||0) - (b.position||0)));
  }
  ...
  const allSelected = listTasks.length > 0 && selectedTasks.size === listTasks.length;
    ...
    setListTasks((tasks) => {
      ...
      if (isOverTask) {
        ...
        if (activeTask.statusId !== overTask.statusId) {
          ...
          newTasks[activeIndex] = { ...newTasks[activeIndex], statusId: overTask.statusId };
      ...
      if (isOverSection) {
        ...
        if (activeTask.statusId !== overId) {
          const newTasks = [...tasks];
          newTasks[activeIndex] = { ...newTasks[activeIndex], statusId: overId as string };
  ...
  const onDragEnd = (event: DragEndEvent) => {
    ...
    const statusTasks = listTasks.filter(t => t.statusId === activeTask.statusId);
    ...
    setListTasks(prevTasks => prevTasks.map(t => t.id === activeId ? { ...t, position: newPosition } : t));
  ...
  return (
    ...
    >
      ...
      <DragOverlay dropAnimation={dropAnimation}>
        {activeTask ? (
          <div className="shadow-2xl bg-background border border-border cursor-grabbing scale-[1.01] rounded-md overflow-hidden">
            <TaskRow
              task={activeTask}
              projectTemplate={projectTemplate}
              onSelectTask={onSelectTask}
