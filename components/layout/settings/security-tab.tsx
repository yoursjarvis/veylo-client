...
export function SecurityTab() {
  ...
  const [twoFactorStep, setTwoFactorStep] = useState<
    "password" | "otp" | "qr" | "verify" | "disable"
  >("password")
  ...
  if (
    auth?.session &&
    !displayedSessions.some((s) => s.id === auth.session?.id)
  ) {
    const ua =
      auth.session.userAgent ||
    ...
    displayedSessions.push({
      ...
      os,
      createdAt:
        (auth.session as Session).createdAt || new Date().toISOString(),
      updatedAt:
        (auth.session as Session).updatedAt || new Date().toISOString(),
      expiresAt:
        (auth.session as Session).expiresAt || new Date().toISOString(),
    })
  }

  return (
    <div className="animate-in space-y-8 pb-20 duration-300 fade-in slide-in-from-bottom-2">
      <div>
        <h3 className="text-lg font-medium">Sessions & Security</h3>
        <p className="text-sm text-muted-foreground">
          Secure your account with two-factor authentication and manage active
          sessions.
        </p>
      </div>
              ...
              >
                ...
                >
                  {(field) => {
                    ...
                    if (passwordValidationErrors.current)
                      fieldErrors.push(passwordValidationErrors.current)
                    const hasError =
                ...
                >
                  {(field) => {
                    ...
                    if (passwordValidationErrors.new)
                      fieldErrors.push(passwordValidationErrors.new)
                    const hasError =
                      field.state.meta.isTouched && !!fieldErrors.length
                    return (
                      <div className="space-y-2">
                ...
                >
                  {(field) => {
                    ...
                    if (passwordValidationErrors.confirm)
                      fieldErrors.push(passwordValidationErrors.confirm)
                    const hasError =
                      field.state.meta.isTouched && !!fieldErrors.length
                    return (
                      <div className="space-y-2">
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <CardDescription>
                  Manage the devices and browsers currently logged into your
                  account.
                </CardDescription>
              </div>
              <Button
              ...
              <div className="space-y-4">
                {[...displayedSessions]
                  ...
                  .map((session) => {
                    ...
                    const isTablet =
                      os.includes("ipad") || os.includes("tablet")
                    const isMac =
                      os.includes("mac") || userAgent.includes("macintosh")
                    ...
                    return (
                      <div
                        key={session.id}
                        className="flex items-start justify-between rounded-xl border bg-muted/20 p-4 transition-colors hover:bg-muted/30"
                      >
                        <div className="flex items-start gap-4">
                          <div className="mt-0.5 rounded-lg border bg-background p-2.5 shadow-sm">
                            <HugeiconsIcon
                              icon={DeviceIcon}
                              size={20}
                          ...
                          <div className="space-y-1">
                            <div className="flex flex-col">
                              ...
                              {isCurrent && (
                                <span className="text-2xs font-medium tracking-wider text-primary uppercase">
                                  This device
                                </span>
                              )}
                            </div>
                        ...
                        {!isCurrent && (
                          <Button
                            variant="ghost"
                            size="icon-sm"
                            className="-mr-1 text-muted-foreground transition-colors hover:text-destructive"
            ...
            {twoFactorStep === "password" ? (
              ...
              <div className="flex w-full flex-col items-center space-y-6">
                ...
                <Button
                  ...
                  disabled={
                    otpCode.length !== 6 ||
                    (auth?.user?.hasPassword && !twoFactorPassword)
                  }
                >
                  Disable 2FA
                </Button>
      ...
      <Dialog open={showBackupCodes} onOpenChange={setShowBackupCodes}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Store these recovery keys in a safe place. They can be used to
          ...
          <div className="grid grid-cols-2 gap-2 py-4">
            ...
            {(backupCodes?.length === 0 || !backupCodes) && (
              <div className="col-span-2 py-4 text-center text-sm text-muted-foreground italic">
                Recovery keys are only shown once during setup or when
                regenerated.
              </div>
            )}
          </div>
