...
const NOTIFICATION_TYPES: NotificationTypeOption[] = [
  {
    ...
    description: "Receive notifications when tasks are assigned to you or when you are removed from them.",
  },
  {
    id: "status_change",
    label: "Task Status Updates",
    description: "Receive notifications when the status of your created or assigned tasks changes.",
  },
  {
    id: "mention",
    label: "Mentions (@)",
    description: "Receive notifications when you are mentioned in task descriptions or comments.",
  },
  {
    id: "comment",
    label: "Task Comments",
    description: "Receive notifications when comments are posted on tasks you are assigned to.",
  },
  {
    id: "reply",
    label: "Replies to Comments",
    description: "Receive notifications when someone replies to a comment you posted.",
  },
  {
    id: "project_added",
  ...
  },
  {
    ...
    description: "Receive notifications when someone reacts with an emoji to your comments.",
  },
];
...
const NOTIFICATION_CHANNELS: NotificationTypeOption[] = [
  {
    ...
    description: "Display notifications inside the application's notification center panel.",
  },
  {
    id: "email",
  ...
  },
  {
    ...
    description: "Trigger desktop notifications through the browser window alert system.",
  },
];
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="space-y-4">
        <div>
          <h4 className="text-sm font-semibold text-foreground uppercase tracking-wider">Delivery Channels</h4>
          <p className="text-xs text-muted-foreground">Toggle how you want notifications delivered.</p>
        </div>
        <div className="space-y-4 max-w-2xl divide-y divide-border border-y border-border">
            ...
            return (
              <div key={option.id} className="flex items-center justify-between py-4 first:pt-0">
                <div className="space-y-0.5 pr-4">
                  <Label htmlFor={`chan-${option.id}`} className="text-sm font-semibold cursor-pointer">
                    {option.label}
                  </Label>
      ...
      <div className="space-y-4">
        <div>
          <h4 className="text-sm font-semibold text-foreground uppercase tracking-wider">Notification Types</h4>
          <p className="text-xs text-muted-foreground">Select which event updates you want to trigger alerts.</p>
        </div>
        <div className="space-y-4 max-w-2xl divide-y divide-border border-y border-border">
          {NOTIFICATION_TYPES.map((option) => {
            ...
            return (
              <div key={option.id} className="flex items-center justify-between py-4 first:pt-0">
                <div className="space-y-0.5 pr-4">
                  <Label htmlFor={`pref-${option.id}`} className="text-sm font-semibold cursor-pointer">
                    {option.label}
                  </Label>
                  <p className="text-xs text-muted-foreground">
                    {option.description}
