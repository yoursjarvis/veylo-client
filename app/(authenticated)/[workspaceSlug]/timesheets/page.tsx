...
export default function TimesheetsPage() {
  ...
  const weeklyTotal = days.reduce(
    (total, _, i) => total + calculateDailyTotal(i),
    0
  )
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-background p-8">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div className="space-y-1.5">
            ...
            <p className="text-sm text-muted-foreground">
              Log your hours across projects and tasks. Ensure your weekly total
              is accurate before submitting.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-3 rounded-lg border border-border bg-muted px-4 py-2 shadow-sm">
        ...
        <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
          {/* Header Row */}
          <div className="grid grid-cols-[2fr_repeat(7,1fr)_1fr] gap-4 border-b border-border bg-muted/50 p-4 text-sm font-semibold text-muted-foreground">
            <div>Project / Task</div>
            {days.map((day, i) => (
              <div key={day} className="text-center">
                <div className="text-xs tracking-wider uppercase">{day}</div>
                <div className="mt-0.5 text-base text-foreground">
                  {dates[i]}
                </div>
              </div>
            ))}
          ...
          <div className="divide-y divide-border">
            {timesheetData.map((task, tIndex) => (
              <div
                key={task.id}
                className="grid grid-cols-[2fr_repeat(7,1fr)_1fr] items-center gap-4 p-4 transition-colors hover:bg-muted/20"
              >
                ...
                {days.map((_, dIndex) => (
                  <div key={dIndex} className="flex justify-center">
                    <Input
                      ...
                      onChange={(e) =>
                        handleHourChange(tIndex, dIndex, e.target.value)
                      }
                      className={cn(
                        "h-9 w-16 text-center text-sm font-medium transition-colors focus:border-primary",
                        parseFloat(task.hours[dIndex]) > 0
          ...
          {/* Footer Row (Totals) */}
          <div className="grid grid-cols-[2fr_repeat(7,1fr)_1fr] gap-4 border-t border-border bg-muted/30 p-4 font-bold">
            <div className="flex items-center justify-end pr-2 text-right text-sm text-muted-foreground">
              Daily Totals
            </div>
            {days.map((_, i) => (
              <div key={i} className="text-center text-sm text-foreground">
                {calculateDailyTotal(i) > 0
                  ? `${calculateDailyTotal(i).toFixed(1)}h`
                  : "-"}
              </div>
            ))}
