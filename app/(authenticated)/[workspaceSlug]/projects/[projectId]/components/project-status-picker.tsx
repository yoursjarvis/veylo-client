  ...
  useEffect(() => {
    if (projectId) {
      const savedStatus = localStorage.getItem(
        `veylo-project-status-${projectId}`
      )
      if (savedStatus) {
  ...
  return (
    <Card className="border border-border bg-card shadow-sm">
      ...
      <CardContent className="space-y-4">
        ...
        <div
          className={`flex items-start gap-3 rounded-xl border p-4 ${current.bgClass}`}
        >
          <div className="mt-0.5 shrink-0">{current.icon}</div>
          <div className="space-y-1">
            <h4 className="text-xs leading-none font-bold tracking-wider uppercase">
              {current.label}
            </h4>
            <p className="text-sm leading-relaxed opacity-90">{current.text}</p>
          </div>
        </div>
      </CardContent>
    </Card>
